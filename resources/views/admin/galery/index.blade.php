...
<div class="bg-gray-800 rounded-lg shadow-lg border border-gray-700 w-full">
    <div class="p-4 border-b border-gray-700 flex items-center justify-between">
        <h2 class="text-xl font-semibold text-white">galeri</h2>
    </div>

    <div class="p-4">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
            <div class="relative max-w-md w-full">
                <input type="text" placeholder="Search galeri..."
            ...
            <a href="{{ route('galery.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 flex items-center gap-2">
                <i class="bi bi-plus-circle"></i> Add galeri
            </a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="px-4 py-3 text-left text-white font-semibold">Iamge</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="text-gray-300">
                        <td class="px-4 py-3">
                            <img src="{{ asset('storage/galery/' . $item->image) }}" alt="" class="w-20 h-20 object-cover">
                        <td class="px-4 py-3">{{ $item->name }}</td>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex gap-2">
                                <a href="{{ route('galery.edit', $item->id) }}" class="p-2 text-blue-500 hover:text-blue-400">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('galery.destroy', $item->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button class="p-2 text-red-500 hover:text-red-400" onclick="return confirm('are you sure delet this galery?')">
